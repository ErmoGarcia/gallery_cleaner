...
class _RecoveryState extends State<Recovery> {
  ...
  final Deleted _db = Deleted();
  final Trash _trash = Trash();
  ...
  _loadMedia() async {
    List<Deleted> deletedList = await _db.select().toList();
  ...
  _emptyTrash() async {
    ...
    final result = await _db.select().delete();
  ...
  _recoverSelected(gallery) async {
    ...
    for (File file in _selected) {
      gallery.add(file);
      final result = await _db.select().path.equals(file.path).delete();
      print(result.toString());
    }
